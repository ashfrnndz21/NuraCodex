...
import { presentTreatmentHistoryEvent } from '../src/services/treatmentHistoryPresentation.mjs';
      ...
      {visible.map((item) => {
        ...
        return <View key={item.id} style={[s.recordCard, expanded && s.recordCardExpanded]}>
          ...
          {expanded && <View style={s.expandedBody}>
            ...
            {expandedId === `history:${item.id}` && <View style={s.historyList}>{history.map((event) => { const provenance = presentTreatmentHistoryEvent(event, assets); const sourceAsset = provenance.sourceAssetId ? assets.find((asset) => asset.id === provenance.sourceAssetId) : undefined; return <View key={event.id} style={s.historyRow}><View style={s.historyDot} /><View style={s.historyCopy}><Text style={s.historyTitle}>{event.summary}</Text><Text style={s.historyDate}>{dateLabel(event.occurredAt)}</Text><Text style={s.historyVersion}>{event.snapshot.dose || 'Dose not provided'}{event.snapshot.schedule ? ` · ${event.snapshot.schedule}` : ''}</Text><Text style={s.historyStatus}>{provenance.statusLabel}{provenance.statusDate ? ` · ${dateLabel(provenance.statusDate)}` : ''}</Text><Text style={s.historySource}>{provenance.sourceKind === 'user_entered' ? 'USER-ENTERED' : provenance.sourceKind === 'attached_source' ? 'SOURCE' : 'SOURCE UNAVAILABLE'} · {provenance.sourceLabel}</Text>{sourceAsset && <Pressable accessibilityRole="button" accessibilityLabel={`Open source ${sourceAsset.name} for this dated treatment version`} onPress={() => router.push({ pathname: '/review', params: { purpose: 'medical', assetId: sourceAsset.id, ...(sourceAsset.serverSourceId ? { sourceId: sourceAsset.serverSourceId } : {}) } })} style={s.historyOpenSource}><Text style={s.openSourceText}>OPEN THIS SOURCE  ↗</Text></Pressable>}</View></View>; })}</View>}
...
const s = StyleSheet.create({
  ...
  historyStatus: { color: C.plum, fontSize: 7, fontWeight: '700', letterSpacing: .35, marginTop: 3 },
  historySource: { color: C.faint, fontSize: 7, lineHeight: 11, marginTop: 3 },
  historyOpenSource: { alignSelf: 'flex-start', marginTop: 2, paddingVertical: 4 },
});
