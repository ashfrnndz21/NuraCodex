...
function TapScale({ children, onPress, label, style, reducedMotion }: { children: React.ReactNode; onPress: () => void; label: string; style: any; reducedMotion: boolean }) {
  ...
  return <Animated.View style={{ transform: [{ scale }] }}><Pressable accessibilityRole="button" accessibilityLabel={label} onPressIn={pressIn} onPressOut={pressOut} onPress={onPress} style={style}>{children}</Pressable></Animated.View>;
}
...
export default function Home() {
  ...
  const recent = useMemo<RecentItem[]>(() => [
    ...
    ...assets.map((asset) => ({ id: asset.id, title: asset.name, detail: asset.kind === 'video' ? 'Video saved · contents not analyzed' : 'Source saved · contents not analyzed', date: asset.addedAt, type: asset.kind.toUpperCase(), source: asset.uri.startsWith('demo:') ? 'Synthetic demo source' : 'Added by you', icon: asset.kind === 'video' ? '▶' : '▤' })),
  ...
  return <View style={styles.page}>
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {storageError ? <View style={styles.storageNotice}><Text style={styles.storageTitle}>Your local profile needs attention</Text><Text style={styles.storageBody}>Some changes may not have been saved. You can continue, but check before leaving this screen.</Text></View> : null}
      ...
      {typeof window !== 'undefined' && <View style={styles.demoNotice}>
        ...
        <View style={styles.demoCopy}><Text style={styles.demoTitle}>Local browser preview</Text><Text style={styles.demoBody}>Synthetic samples are stored in this browser. Your changes stay in this tab until refresh. Don’t enter real health or contact details.</Text>
          {confirmReset ? <View style={styles.resetConfirm}><Text style={styles.resetPrompt}>Reset this local preview?</Text><Pressable accessibilityRole="button" onPress={() => { resetDemo(); setConfirmReset(false); }}><Text style={styles.resetAction}>Reset</Text></Pressable><Pressable accessibilityRole="button" onPress={() => setConfirmReset(false)}><Text style={styles.resetCancel}>Keep</Text></Pressable></View> : <Pressable accessibilityRole="button" accessibilityLabel="Reset local browser preview" onPress={() => setConfirmReset(true)}><Text style={styles.resetLink}>Reset demo</Text></Pressable>}
        </View>
      </View>}
      ...
      <View style={styles.countStrip}>
        ...
        <Count value={facts.filter((fact) => !fact.validUntil).length} label="HEALTH DETAILS" tint="#BBD3FA" ink="#1767D8" />
        <View style={styles.countDivider} />
      ...
      <View style={styles.sectionHead}>
        ...
        <Pressable accessibilityRole="button" onPress={() => router.push('/(tabs)/health')} style={styles.textLink}><Text style={styles.textLinkText}>Open map</Text><Arrow /></Pressable>
      </View>
      <View style={styles.domainGrid}>
        {domains.map((domain) => {
          const count = domain.label === 'Records' ? assets.length : domain.label === 'Treatment' ? treatments.length : domain.label === 'Care' ? visits.length : facts.filter((fact) => !fact.validUntil && domain.match.test(fact.category)).length;
          const selected = topics.some((topic) => domain.match.test(topic.label)) || count > 0;
          return <TapScale key={domain.label} reducedMotion={reducedMotion} onPress={() => router.push(domain.label === 'Cover' ? '/insurance' : domain.label === 'Treatment' ? '/treatment' : domain.label === 'Care' ? '/visits' : '/(tabs)/health')} label={domain.detail + ', ' + count + ' saved'} style={[styles.domainCard, selected && styles.domainCardActive]}>
            <View style={[styles.domainMark, { backgroundColor: domain.tint }]}><View style={[styles.domainDotSmall, { backgroundColor: selected ? domain.ink : '#C8C5CE' }]} /></View>
            <View style={styles.domainText}><Text style={styles.domainTitle}>{domain.label}</Text><Text style={styles.domainDetail}>{domain.detail}</Text></View>
            <Text style={[styles.domainCount, selected && { color: domain.ink }]}>{String(count).padStart(2, '0')}</Text>
          </TapScale>;
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  brand: { flexDirection: 'row', alignItems: 'center', gap: 9 }, wordmark: { color: colors.ink, fontSize: 20, fontWeight: '700', letterSpacing: -0.7 }, tagline: { color: colors.quiet, fontSize: 7, fontWeight: '700', letterSpacing: 1.5, marginTop: 2 },
  profileButton: { width: 39, height: 39, borderRadius: 20, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: colors.border, alignItems: 'center', justifyContent: 'center', ...shadow }, profileButtonText: { color: colors.violet, fontSize: 15, fontWeight: '600' }, pressed: { opacity: .9, transform: [{ scale: motion.pressScale }] },
  ...
  hero: { minHeight: 188, borderRadius: 24, overflow: 'hidden', backgroundColor: '#EEF1F8', padding: 18, borderWidth: 1, borderColor: '#E4E1E8', flexDirection: 'row', marginBottom: 14 },
  heroBloom: { position: 'absolute', right: -55, top: -75, width: 218, height: 218, borderRadius: 110, backgroundColor: '#E9DDF2', opacity: .72 },
  heroContent: { flex: 1, paddingRight: 74, zIndex: 1 }, heroKicker: { flexDirection: 'row', alignItems: 'center', gap: 7 }, heroDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#1767D8' }, heroKickerText: { color: '#615777', fontSize: 8, fontWeight: '800', letterSpacing: 1.15 },
  heroTitle: { color: colors.ink, fontSize: 25, lineHeight: 29, fontWeight: '400', letterSpacing: -.8, marginTop: 11 }, heroBody: { color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 7, maxWidth: 245 },
  heroAction: { minHeight: 34, flexDirection: 'row', alignItems: 'center', gap: 7, alignSelf: 'flex-start', paddingVertical: 5, paddingHorizontal: 9, marginTop: 10, borderRadius: 14, backgroundColor: 'rgba(255,255,255,.72)' }, heroActionText: { color: '#275CA8', fontSize: 10, fontWeight: '700' }, arrow: { color: '#1767D8', fontSize: 16, lineHeight: 19 }, heroOrb: { position: 'absolute', right: 8, top: 39, width: 92, height: 92, alignItems: 'center', justifyContent: 'center' }, orbHalo: { position: 'absolute', height: 88, width: 88, borderRadius: 44, borderWidth: 1, borderColor: 'rgba(118,98,154,.20)', backgroundColor: 'rgba(255,255,255,.18)' },
  ...
  demoIcon: { width: 18, height: 18, borderRadius: 9, backgroundColor: '#DFD4EA', alignItems: 'center', justifyContent: 'center', marginTop: 1 }, demoIconText: { color: '#634C76', fontSize: 10, fontWeight: '700' }, demoCopy: { flex: 1 }, demoTitle: { color: '#4C3C5B', fontSize: 10, fontWeight: '700' }, demoBody: { color: colors.muted, fontSize: 9, lineHeight: 13, marginTop: 2 }, resetLink: { color: '#1767D8', fontSize: 9, fontWeight: '700', marginTop: 4 }, resetConfirm: { flexDirection: 'row', alignItems: 'center', gap: 14, marginTop: 6 }, resetPrompt: { color: colors.ink, fontSize: 9, flex: 1 }, resetAction: { color: '#A44D48', fontSize: 9, fontWeight: '700' }, resetCancel: { color: colors.muted, fontSize: 9, fontWeight: '600' },
  countStrip: { flexDirection: 'row', alignItems: 'center', borderRadius: 17, borderWidth: 1, borderColor: colors.border, backgroundColor: '#FFFFFF', paddingVertical: 11, marginBottom: 22, ...shadow }, countCell: { flex: 1, alignItems: 'center', gap: 2 }, countValue: { fontSize: 19, fontWeight: '500' }, countLabel: { color: colors.quiet, fontSize: 6.5, fontWeight: '700', letterSpacing: .65, textAlign: 'center' }, countRule: { height: 2, width: 19, borderRadius: 2, marginTop: 4 }, countDivider: { width: 1, height: 32, backgroundColor: colors.border },
  sectionHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 10 }, eyebrow: { color: colors.violet, fontSize: 8, fontWeight: '800', letterSpacing: 1.25 }, sectionTitle: { color: colors.ink, fontSize: 18, fontWeight: '500', letterSpacing: -.35, marginTop: 4 }, textLink: { flexDirection: 'row', alignItems: 'center', gap: 2, paddingBottom: 2 }, textLinkText: { color: '#1767D8', fontSize: 9, fontWeight: '700' },
  domainGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 7, marginBottom: 23 }, domainCard: { width: '32%', minHeight: 82, paddingHorizontal: 8, paddingVertical: 8, borderRadius: 15, borderWidth: 1, borderColor: colors.border, backgroundColor: '#FFFFFF', flexDirection: 'row', alignItems: 'flex-start', gap: 6, ...shadow }, domainCardActive: { borderColor: '#D7D2DE', backgroundColor: '#FFFFFF' }, domainMark: { width: 17, height: 17, borderRadius: 9, alignItems: 'center', justifyContent: 'center' }, domainDotSmall: { width: 6, height: 6, borderRadius: 3 }, domainText: { flex: 1 }, domainTitle: { color: colors.ink, fontSize: 9, fontWeight: '700' }, domainDetail: { color: colors.quiet, fontSize: 7, lineHeight: 9, marginTop: 3 }, domainCount: { color: '#B2AFB8', fontSize: 8, fontWeight: '700' },
  sectionHeadRecent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 2, marginBottom: 10 }, smallArrow: { width: 30, height: 30, borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center' },
  recentList: { marginBottom: 20 }, recentCard: { minHeight: 73, borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: '#FFFFFF', paddingHorizontal: 10, paddingVertical: 10, flexDirection: 'row', alignItems: 'center', gap: 9, ...shadow }, recentCardNext: { marginTop: 7 }, recentDate: { width: 30, alignItems: 'center' }, recentDateDay: { color: colors.ink, fontSize: 14, fontWeight: '600' }, recentDateMonth: { color: colors.quiet, fontSize: 6, fontWeight: '700', letterSpacing: .7, marginTop: 1 }, recentNode: { width: 35, height: 35, borderRadius: 18, backgroundColor: '#1264F5', borderWidth: 1, borderColor: '#A7C5FF', alignItems: 'center', justifyContent: 'center' }, recentNodeGlyph: { color: '#FFFFFF', fontSize: 16 }, recentText: { flex: 1 }, recentMeta: { flexDirection: 'row', alignItems: 'center', gap: 4 }, recentType: { color: '#1767D8', fontSize: 6.5, fontWeight: '800', letterSpacing: .7 }, recentMetaDot: { color: colors.quiet, fontSize: 7 }, recentSource: { color: colors.quiet, fontSize: 7, flex: 1 }, recentTitle: { color: colors.ink, fontSize: 11, fontWeight: '600', marginTop: 3 }, recentDetail: { color: colors.muted, fontSize: 8, marginTop: 2 },
  emptyRecord: { padding: 16, borderRadius: 17, borderWidth: 1, borderColor: colors.border, backgroundColor: '#FFFFFF', marginBottom: 16 }, emptyRecordTitle: { color: colors.ink, fontSize: 13, fontWeight: '600' }, emptyRecordBody: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 5 }, emptyRecordLink: { color: '#1767D8', fontSize: 10, fontWeight: '700', marginTop: 9 },
  questionCard: { backgroundColor: '#FFFFFF', padding: 14, borderRadius: 17, borderWidth: 1, borderColor: colors.border, marginBottom: 19 }, questionText: { color: colors.ink, fontSize: 12, lineHeight: 17, marginTop: 6 }, questionLink: { color: '#1767D8', fontSize: 9, fontWeight: '700', marginTop: 7 },
  actions: { flexDirection: 'row', gap: 8, marginBottom: 16 }, actionCard: { flex: 1, minHeight: 127, borderRadius: 17, borderWidth: 1, borderColor: colors.border, backgroundColor: '#FFFFFF', padding: 12, ...shadow }, actionAsk: { backgroundColor: '#F5F1F8' }, actionGlyphWrap: { width: 32, height: 32, borderRadius: 14, alignItems: 'center', justifyContent: 'center' }, actionGlyph: { fontSize: 20, fontWeight: '300' }, actionTitle: { color: colors.ink, fontSize: 11, fontWeight: '700', marginTop: 10 }, actionSub: { color: colors.muted, fontSize: 8, lineHeight: 12, marginTop: 3 }, footerNote: { color: colors.quiet, fontSize: 8, lineHeight: 12, textAlign: 'center', paddingHorizontal: 8, marginTop: 4 },
  visitCallout: { flexDirection: 'row', alignItems: 'center', gap: 10, minHeight: 64, borderRadius: 17, borderWidth: 1, borderColor: '#E6DCEB', backgroundColor: '#F4EEF6', paddingHorizontal: 12, paddingVertical: 10, marginBottom: 16 }, visitCalloutNode: { width: 34, height: 34, borderRadius: 13, alignItems: 'center', justifyContent: 'center', backgroundColor: '#E7D8EB' }, visitCalloutGlyph: { color: '#735A83', fontSize: 18 }, visitCalloutTitle: { color: colors.ink, fontSize: 10, fontWeight: '700' }, visitCalloutBody: { color: colors.muted, fontSize: 8, lineHeight: 12, marginTop: 2 }, visitCalloutArrow: { color: '#735A83', fontSize: 19 }, symptomCallout: { backgroundColor: '#FFF7EA', borderColor: '#EADCC7' }, symptomNode: { backgroundColor: '#F3E4CC' }, symptomGlyph: { color: '#865B20', fontWeight: '800' }, symptomArrow: { color: '#865B20' },
});
