...
import { compactFeedBrief, summarizeFeedActivity } from '../../src/services/feedPresentation.mjs';
...
function FeedCard({ item, index, reducedMotion, brief, showHidden, onSave, onDismiss, onRestore, onAsk, onOpen }: {
  item: HealthFeedItem; index: number; reducedMotion: boolean; brief?: FeedBrief; showHidden: boolean; onSave: () => void; onDismiss: () => void; onRestore: () => void; onAsk: () => void; onOpen: () => void;
}) {
  ...
  return <Animated.View style={{ opacity, transform: [{ translateY: rise }] }}>
    <Surface style={styles.feedCard}>
      ...
      <View style={styles.articleActions}>
        {showHidden ? <>
          <Pressable accessibilityRole="button" accessibilityLabel="Restore article to For you" onPress={onRestore} style={[styles.actionPill, styles.restorePill]}><Text style={styles.restoreText}>↶ Restore to For you</Text></Pressable>
...
function FeedActivityRow({ item, busy, reducedMotion }: { item: ReturnType<typeof summarizeFeedActivity>[number]; busy: boolean; reducedMotion: boolean }) {
  ...
  const [checkScale] = useState(() => new Animated.Value(item.status === 'complete' ? 1 : 0.82));
  useEffect(() => {
  ...
  return <Animated.View style={[styles.activityRow, { opacity, transform: [{ translateY: rise }] }]}>
    <Animated.View style={[styles.activityDot, item.status === 'complete' && styles.activityDotDone, { transform: [{ scale: checkScale }] }]}>{item.status === 'complete' && <Text style={styles.activityCheck}>✓</Text>}</Animated.View>
    <View style={styles.activityCopy}><Text style={styles.activityLabel}>{item.topic}</Text><Text style={styles.activityDetail}>{item.detail}</Text></View>
    {item.status === 'started' && busy ? <ActivityIndicator color={colors.violet} size="small" /> : null}
  </Animated.View>;
}
...
function SearchBriefCard({ brief, reducedMotion }: { brief: FeedBrief; reducedMotion: boolean }) {
  ...
  return <Animated.View style={{ opacity }}><Surface style={styles.briefCard}>
    <View style={styles.briefHeader}><View><Label>NU﻿RA’S SEARCH BRIEF</Label><Text style={styles.briefTitle}>{brief.topic}, in a nutshell.</Text></View><View style={styles.briefSpark}><Text style={styles.briefSparkText}>✦</Text></View></View>
...
export default function Services() {
  ...
  const visibleItems = useMemo(() => groupedItems.filter((item) => feedView === 'saved' ? item.saved : feedView === 'hidden' ? item.dismissed : !item.dismissed).sort((a, b) => Number(b.saved) - Number(a.saved) || b.retrievedAt.localeCompare(a.retrievedAt)), [groupedItems, feedView]);
  ...
  return <View style={styles.page}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {briefs.length > 0 && feedView === 'forYou' ? <View style={styles.briefList}>{briefs.map((brief) => <SearchBriefCard key={brief.id} brief={brief} reducedMotion={reducedMotion} />)}</View> : null}
    <View style={styles.feedHeader}><View><Label>YOUR READING</Label><Text style={styles.feedTitle}>{feedView === 'saved' ? 'Saved for later.' : feedView === 'hidden' ? 'Hidden from your feed.' : 'For your health journey.'}</Text></View><View style={styles.segment}>
