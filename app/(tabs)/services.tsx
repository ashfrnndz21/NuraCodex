...
export default function Services() {
  ...
  useEffect(() => {
    ...
    getAgentStatus().then((status) => {
      ...
      setServiceReason(ready ? '' : status.reason || 'Trusted health search is disabled on Nura’s local service.');
    });
  ...
  const visibleItems = useMemo(() => feedItems.filter((item) => showSaved ? item.saved : !item.dismissed).sort((a, b) => Number(b.saved) - Number(a.saved) || b.retrievedAt.localeCompare(a.retrievedAt)), [feedItems, showSaved]);
  const savedCount = feedItems.filter((item) => item.saved).length;
  ...
  async function search() {
    ...
    setBusy(true); setError(''); setActivity([]); setBriefs([]); setUndoId(null);
    try {
  ...
  function dismiss(item: HealthFeedItem) { setFeedDismissed(item.id, true); setUndoId(item.id); }
  function undoDismiss() { if (undoId) setFeedDismissed(undoId, false); setUndoId(null); }
  ...
  return <View style={styles.page}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <Surface style={styles.consentCard}>
      ...
      <View style={styles.sourceTrust}><View style={[styles.statusDot, serviceStatus === 'ready' && styles.statusDotOn, serviceStatus === 'unavailable' && styles.statusDotOff]} /><Text style={styles.sourceTrustText}>{serviceStatus === 'checking' ? 'Checking trusted-source search…' : serviceStatus === 'ready' ? 'Sources limited to WHO, CDC, NHS, MedlinePlus and other trusted publishers.' : serviceReason}</Text><Pressable accessibilityRole="button" accessibilityLabel="Check trusted source search" onPress={() => { setServiceStatus('checking'); getAgentStatus().then((status) => { const ready = status.available && status.capabilities?.trustedHealthSearch === true; setServiceStatus(ready ? 'ready' : 'unavailable'); setServiceReason(ready ? '' : status.reason || 'Trusted health search is disabled on Nura’s local service.'); }); }}><Text style={styles.refresh}>↻</Text></Pressable></View>
    ...
    {undoId ? <View style={styles.undoBar}><Text style={styles.undoText}>Removed from your feed.</Text><Pressable accessibilityRole="button" onPress={undoDismiss}><Text style={styles.undoAction}>Undo</Text></Pressable></View> : null}
...
const styles = StyleSheet.create({
  ...
  consentCard: { padding: 15, marginBottom: 13 }, consentTop: { flexDirection: 'row', gap: 10, alignItems: 'flex-start' }, consentIcon: { width: 35, height: 35, borderRadius: 13, backgroundColor: '#EAF1FD', alignItems: 'center', justifyContent: 'center' }, consentIconText: { fontSize: 23, lineHeight: 25, color: '#1767D8' }, consentTitle: { color: colors.ink, fontSize: 14, fontWeight: '600', lineHeight: 18 }, consentBody: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 4 }, topicLabelRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 14 }, selectedCount: { color: colors.violet, fontSize: 9, fontWeight: '600' }, topicList: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 8 }, noTopics: { backgroundColor: '#F5F3F7', borderRadius: 13, padding: 11, marginTop: 9 }, noTopicsText: { color: colors.muted, fontSize: 10, lineHeight: 15 }, inlineLink: { color: '#1767D8', fontSize: 10, fontWeight: '700', marginTop: 7 }, limitNote: { color: '#9A6B24', fontSize: 9, marginTop: 2 }, consentToggle: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, paddingVertical: 11, borderTopWidth: 1, borderTopColor: colors.border, marginTop: 3 }, checkBox: { width: 19, height: 19, borderRadius: 6, borderWidth: 1.5, borderColor: '#B7AFBF', backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center' }, checkBoxOn: { backgroundColor: '#1767D8', borderColor: '#1767D8' }, checkMark: { color: '#FFFFFF', fontSize: 12, fontWeight: '800' }, consentToggleText: { flex: 1, color: colors.text, fontSize: 10, lineHeight: 15, paddingTop: 1 }, sourceTrust: { flexDirection: 'row', alignItems: 'flex-start', gap: 7, backgroundColor: '#F4F2F6', paddingHorizontal: 9, paddingVertical: 8, borderRadius: 11 }, statusDot: { width: 7, height: 7, borderRadius: 4, marginTop: 3, backgroundColor: '#D2CCD7' }, statusDotOn: { backgroundColor: '#34845B' }, statusDotOff: { backgroundColor: '#BD8842' }, sourceTrustText: { flex: 1, color: colors.muted, fontSize: 9, lineHeight: 13 }, refresh: { color: colors.violet, fontSize: 17, lineHeight: 16, paddingHorizontal: 2 }, searchButton: { minHeight: 46, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, backgroundColor: '#1767D8', borderRadius: 15, marginTop: 11, ...shadow }, searchDisabled: { backgroundColor: '#9AB7E9', opacity: 0.76 }, searchButtonText: { color: '#FFFFFF', fontSize: 11, fontWeight: '700' }, searchArrow: { color: '#FFFFFF', fontSize: 19, marginTop: -1 }, safetyNote: { color: colors.quiet, fontSize: 8, lineHeight: 12, textAlign: 'center', marginTop: 8 },
  activityCard: { padding: 14, marginBottom: 13, borderColor: '#D8E5FB' }, activityHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 7 }, activityTitle: { color: colors.ink, fontSize: 14, fontWeight: '600', marginTop: 5 }, activityRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 6 }, activityDot: { width: 17, height: 17, borderRadius: 9, borderWidth: 1.5, borderColor: '#BFC9D9', alignItems: 'center', justifyContent: 'center' }, activityDotDone: { backgroundColor: '#DDF2E8', borderColor: '#94C9A9' }, activityCheck: { fontSize: 10, color: '#287954', fontWeight: '700' }, activityCopy: { flex: 1 }, activityLabel: { color: colors.text, fontSize: 10, fontWeight: '600' }, activityDetail: { color: colors.quiet, fontSize: 8, marginTop: 2 },
  errorCard: { backgroundColor: '#FFF7EB', borderRadius: 15, borderColor: '#E8D8BC', borderWidth: 1, padding: 13, marginBottom: 12 }, errorTitle: { color: '#76501E', fontSize: 12, fontWeight: '700' }, errorBody: { color: '#795F3E', fontSize: 10, lineHeight: 15, marginTop: 4 }, retryHint: { color: '#1767D8', fontSize: 9, fontWeight: '700', marginTop: 8 }, undoBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 10, borderRadius: 12, backgroundColor: '#F0EAF5', marginBottom: 13 }, undoText: { color: colors.violet, fontSize: 10 }, undoAction: { color: colors.violet, fontSize: 10, fontWeight: '700', paddingHorizontal: 6 },
  briefList: { gap: 9, marginBottom: 14 }, briefCard: { padding: 14, backgroundColor: '#F3EEF7', borderColor: '#E4D9EC' }, briefTitle: { color: colors.ink, fontSize: 14, fontWeight: '600', marginTop: 6 }, briefText: { color: colors.muted, fontSize: 10, lineHeight: 16, marginTop: 6 }, briefSources: { color: colors.violet, fontSize: 8, fontWeight: '700', marginTop: 9 },
  feedHeader: { flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', gap: 8, marginTop: 7, marginBottom: 11 }, feedTitle: { color: colors.ink, fontSize: 18, fontWeight: '500', marginTop: 5 }, segment: { flexDirection: 'row', backgroundColor: '#ECE9EF', borderRadius: 13, padding: 3, borderWidth: 1, borderColor: colors.border }, segmentItem: { minHeight: 28, borderRadius: 10, justifyContent: 'center', paddingHorizontal: 9 }, segmentActive: { backgroundColor: '#FFFFFF', ...shadow }, segmentText: { color: colors.muted, fontSize: 9, fontWeight: '500' }, segmentTextActive: { color: colors.ink, fontWeight: '700' }, articleList: { gap: 11 }, feedCard: { padding: 14 }, articleTop: { flexDirection: 'row', alignItems: 'flex-start', gap: 9 }, sourceMark: { width: 31, height: 31, borderRadius: 12, backgroundColor: '#EAF1FD', borderColor: '#D5E3FA', borderWidth: 1, alignItems: 'center', justifyContent: 'center' }, sourceMarkText: { color: '#1767D8', fontSize: 17 }, articleHeading: { flex: 1 }, articleTitleButton: { alignSelf: 'stretch' }, detailToggle: { color: '#1767D8', fontSize: 8, fontWeight: '700', marginTop: 5 }, detailsPanel: { marginTop: 11 }, detailsDivider: { height: 1, backgroundColor: colors.border, marginBottom: 10 }, detailsLabel: { color: colors.violet, fontSize: 8, fontWeight: '800', letterSpacing: 1.05, marginTop: 3 }, detailsCopy: { color: colors.text, fontSize: 10, lineHeight: 16, marginTop: 5 }, detailsWhy: { backgroundColor: '#F5F0F6', borderRadius: 12, padding: 10, marginTop: 10 }, detailsBrief: { backgroundColor: '#F2F6FD', borderRadius: 12, padding: 10, marginTop: 10 }, detailsMeta: { color: colors.quiet, fontSize: 8, lineHeight: 13, marginTop: 8 }, detailsSourceButton: { minHeight: 38, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderRadius: 12, backgroundColor: '#1767D8', marginTop: 11, paddingHorizontal: 12 }, detailsSourceText: { color: '#FFFFFF', fontSize: 9, fontWeight: '800', letterSpacing: .5 }, detailsSourceArrow: { color: '#FFFFFF', fontSize: 14 }, publisher: { color: colors.violet, fontSize: 8, textTransform: 'uppercase', letterSpacing: 0.7, fontWeight: '700' }, articleTitle: { color: colors.ink, fontSize: 14, lineHeight: 19, fontWeight: '600', marginTop: 3 }, articleType: { paddingHorizontal: 7, paddingVertical: 4, backgroundColor: '#EAF1FD', borderRadius: 8 }, articleTypeText: { color: '#1767D8', fontSize: 7, letterSpacing: 0.6, fontWeight: '800' }, articleDetail: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 10 }, articleWhy: { flexDirection: 'row', alignItems: 'center', gap: 6, padding: 8, backgroundColor: '#F5F0F6', borderRadius: 10, marginTop: 9 }, whySpark: { color: '#8A6AA3', fontSize: 11 }, whyText: { flex: 1, color: '#665475', fontSize: 9, lineHeight: 13 }, sourceMeta: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 5, marginTop: 8 }, sourceMetaText: { color: colors.quiet, fontSize: 8 }, sourceMetaDot: { color: colors.quiet, fontSize: 8 }, articleActions: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 12 }, actionPill: { minHeight: 30, justifyContent: 'center', paddingHorizontal: 10, borderRadius: 15, backgroundColor: '#F3F1F5', borderWidth: 1, borderColor: '#E6E1E9' }, actionPillSaved: { backgroundColor: '#E8F4F1', borderColor: '#C6E6D8' }, actionPillText: { color: colors.muted, fontSize: 9, fontWeight: '600' }, actionPillSavedText: { color: '#287954' }, dismissButton: { marginLeft: 'auto', paddingHorizontal: 8, paddingVertical: 6 }, dismissText: { color: colors.quiet, fontSize: 9 }, openSource: { flexDirection: 'row', alignItems: 'center', gap: 4, alignSelf: 'flex-start', marginTop: 9, paddingVertical: 3 }, openSourceText: { color: '#1767D8', fontSize: 9, fontWeight: '700' }, openSourceArrow: { color: '#1767D8', fontSize: 12 }, emptyCard: { alignItems: 'center', paddingVertical: 24, paddingHorizontal: 20 }, emptyOrb: { width: 40, height: 40, borderRadius: 22, backgroundColor: '#F0EAF5', alignItems: 'center', justifyContent: 'center' }, emptyOrbGlyph: { color: colors.violet, fontSize: 18 }, emptyTitle: { color: colors.ink, fontSize: 14, fontWeight: '600', textAlign: 'center', marginTop: 10 }, emptyBody: { color: colors.muted, fontSize: 10, lineHeight: 15, textAlign: 'center', marginTop: 5 }, footer: { color: colors.quiet, fontSize: 8, lineHeight: 13, textAlign: 'center', marginTop: 17, paddingHorizontal: 8 },
});
