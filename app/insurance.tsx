  ...
  return <View style={styles.page}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {policies.length ? policies.map((policy) => {
      ...
      {snapshot.keyDetails.length > 0 && <View style={styles.keyDetailsCard}>
        ...
        {snapshot.keyDetails.slice(0, 4).map(({ key, label, term }) => <Pressable key={`${key}:${term.id ?? term.label}`} accessibilityRole="button" accessibilityLabel={`Open source quote for ${term.label}`} onPress={() => openPolicyTermEvidence(term)} style={({ pressed }) => [styles.keyDetailRow, pressed && styles.keyDetailPressed]}>
          <View style={styles.keyDetailCopy}><Text style={styles.keyDetailLabel}>{label}</Text><Text style={styles.keyDetailValue}>{displayTermValue(term)}</Text></View><Text style={styles.keyDetailArrow}>↗</Text>
        </Pressable>)}
        {snapshot.keyDetails.length > 4 && <Text style={styles.keyDetailsFooter}>The full breakdown below includes {snapshot.keyDetails.length - 4} more key term{snapshot.keyDetails.length - 4 === 1 ? '' : 's'}.</Text>}
      </View>}
