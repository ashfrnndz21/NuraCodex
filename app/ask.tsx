...
export default function Ask() {
  ...
  const selectedContext = policyReviewMode
    ...
    : { facts: selectAskHealthFacts(scopedContext.facts, shareFacts, excludedHealthFactIds), topics: shareTopics ? scopedContext.topics : [], links: shareLinks ? scopedContext.links : [] };
  ...
  function startQuestion() { if (!question.trim() || busy || proposalSaveLock.current) return; setError(''); setShareExternalSearch(false); setShareTreatments(false); setShareVisits(false); setExcludedHealthFactIds([]); setFactSelectionExpanded(false); if (policyReviewMode) setSelectedHealthFactIds([]); setConsentOpen(true); }
...
const s = StyleSheet.create({
  ...
  factDisclosure: { minHeight: 38, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8, borderBottomWidth: 1, borderBottomColor: 'rgba(255, 249, 246, 0.09)' }, factDisclosureTitle: { color: C.plum, fontSize: 9, fontWeight: '600' }, factDisclosureCount: { color: C.faint, fontSize: 8 },
