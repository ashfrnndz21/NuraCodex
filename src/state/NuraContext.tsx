...
export function NuraProvider({ children }: { children: React.ReactNode }) {
  ...
  const clearAllLocalData = useCallback(async () => {
    ...
    setTopics([]); setAssets([]); setFacts([]); setTreatments([]); setTreatmentEvents([]); setVisits([]); setVisitEvents([]); setLinks([]); setFeedItems([]); setSavedQuestions([]); setAgentMessages([]); setRegistryBriefs([]); setStorageError(null);
