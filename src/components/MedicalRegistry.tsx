...
import { registryBriefDisplayText, registryBriefIsCurrent, registryEvidenceSnapshot } from '../services/registryBrief.mjs';
        ...
        {topic && <>
          ...
          <View style={s.briefCard}>
            ...
            {latestBrief ? <>
              ...
              {latestBrief.citations.map((citation) => <Pressable key={`${latestBrief.id}:${citation.id}`} accessibilityRole="button" onPress={() => openHistory(connected.find((item) => item.id === citation.id || item.id.endsWith(`:${citation.id}`))?.id ?? citation.id)} style={({ pressed }) => [s.briefCitation, pressed && s.pressed]}><Text style={s.briefCitationRef}>{citation.reference}</Text><View style={{ flex: 1 }}><Text style={s.briefCitationTitle}>{citation.title}</Text><Text style={s.briefCitationDetail}>{citation.source}{citation.date ? ` · ${citation.date}` : ''}</Text></View><Text style={s.briefArrow}>›</Text></Pressable>)}
