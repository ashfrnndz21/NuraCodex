...
type Entry = { id: string; nodeId: string; kind: 'fact' | 'asset' | 'treatment' | 'visit'; title: string; detail: string; date: string; timestamp: number; source: string; category: string; note?: string; repeat?: boolean; supersedesId?: string; validUntil?: string | null; sourceId?: string; sourceClaimId?: string; sourceAssetName?: string; eventDateKey?: string | null };
type TimelineEntry = Entry & { members?: Entry[] };
...
const C = { canvas: colors.bg, white: colors.surface, ink: colors.ink, muted: colors.muted, faint: colors.quiet, border: colors.border, cobalt: colors.cobalt, cobaltDeep: '#1557B7', blueLine: '#B6A6BE', bluePale: '#EAE3EF', plum: colors.plum, lilac: colors.lilac, lilacInk: colors.violet, mint: colors.mint, peach: colors.peach, track: '#E7DDE9' };
...
function sourceAssetNameFor(sourceId: string | undefined, assets: IntakeAsset[]) {
  ...
  return resolveTimelineSourceAsset(sourceId, assets)?.name;
}
...
export function HealthHistory({ name, ready, storageError, facts, assets, treatments, visits, topics, links, addLink, removeLink, correctFact, reconcileSourceFactDate, attachSourceToAsset, initialFocusId, initialFilter }: Props) {
  ...
  const [pendingFactFocusId, setPendingFactFocusId] = useState<string | null>(null);
  ...
  const entries = useMemo<Entry[]>(() => [
    ...facts.map((fact) => ({ id: `fact:${fact.id}`, nodeId: `fact:${fact.id}`, kind: 'fact' as const, title: fact.label, detail: fact.value, date: readableDate(fact.date), timestamp: dateTime(fact.date), source: fact.source, category: fact.category, note: fact.note, supersedesId: fact.supersedesId, validUntil: fact.validUntil, sourceId: fact.sourceId, sourceClaimId: fact.sourceClaimId, sourceAssetName: sourceAssetNameFor(fact.sourceId, assets), eventDateKey: healthDateKey(fact.date) })),
    ...assets.map((asset) => ({ id: `asset:${asset.id}`, nodeId: `asset:${asset.id}`, kind: 'asset' as const, title: asset.name, detail: `${asset.kind.toUpperCase()} · saved privately`, date: readableDate(asset.addedAt), timestamp: dateTime(asset.addedAt), source: 'Added by you', category: asset.kind.toUpperCase(), sourceId: asset.serverSourceId, eventDateKey: healthDateKey(asset.addedAt), note: asset.possibleRepeat ? 'This looks similar to another file you added. It has not been removed or merged.' : undefined, repeat: asset.possibleRepeat })),
    ...treatments.map((item) => ({ id: `treatment:${item.id}`, nodeId: `treatment:${item.id}`, kind: 'treatment' as const, title: item.name, detail: [item.dose, item.schedule].filter(Boolean).join(' · ') || 'Dose and schedule not provided', date: item.startedOn ? readableDate(item.startedOn) : readableDate(item.createdAt), timestamp: dateTime(item.startedOn || item.createdAt), source: item.source, category: 'Treatment', sourceId: item.sourceId, eventDateKey: healthDateKey(item.startedOn || item.createdAt), sourceAssetName: sourceAssetNameFor(item.sourceId, assets), note: `${item.status === 'current' ? 'Current record' : `Past record${item.endedOn ? ` · marked ${readableDate(item.endedOn)}` : ''}`} · ${item.purpose || 'Purpose not provided'}` })),
    ...visits.map((item) => ({ id: `visit:${item.id}`, nodeId: `visit:${item.id}`, kind: 'visit' as const, title: item.purpose || 'Care visit', detail: [item.clinician, item.location].filter(Boolean).join(' · ') || (item.status === 'upcoming' ? 'Visit planned' : 'Visit captured'), date: item.appointmentAt ? readableDate(item.appointmentAt) : 'Unscheduled', timestamp: dateTime(item.appointmentAt || item.createdAt), source: item.source, category: 'Care visit', eventDateKey: item.appointmentAt ? healthDateKey(item.appointmentAt) : null, note: item.outcome || item.followUp || (item.status === 'upcoming' ? `${item.questions.length} questions saved · ${item.briefFactIds.length + item.briefAssetIds.length + item.briefTreatmentIds.length} records selected for the brief` : 'Outcome not added yet') })),
  ...
  const filteredEntries = useMemo(() => (filter === 'Documents' ? entries : timelineEntries).filter((entry) => {
    ...
    return true;
  }), [entries, filter, timelineEntries]);
  const visibleEntries = useMemo<TimelineEntry[]>(() => filter === 'Documents' ? filteredEntries : groupSourceFactEvents(filteredEntries, assets), [filter, filteredEntries, assets]);
  const timelineSections = useMemo(() => groupTimelineByYear(visibleEntries), [visibleEntries]);
  ...
  useEffect(() => {
    ...
    setFilter(entry.kind === 'asset' ? 'Documents' : 'Everything');
  }, [initialFocusId, ready, entries, timelineEvents]);
  useEffect(() => {
    if (!pendingFactFocusId) return;
    const event = timelineEvents.find((item) => item.nodeId === pendingFactFocusId || item.members?.some((member) => member.nodeId === pendingFactFocusId));
    if (!event) return;
    setSelectedNode(pendingFactFocusId);
    setExpanded(event.id);
    setFilter('Everything');
    setPendingFactFocusId(null);
  }, [pendingFactFocusId, timelineEvents]);
  ...
  const captured = timelineEvents.length;
  ...
  async function openSource(entry: Entry) {
    ...
    const asset = entry.kind === 'asset'
      ...
      : resolveTimelineSourceAsset(fact?.sourceId ?? treatment?.sourceId, assets);
    const sourceId = entry.kind === 'asset' ? asset?.serverSourceId ?? null : asset ? asset.serverSourceId ?? null : fact?.sourceId ?? treatment?.sourceId ?? null;
  ...
  function reviewFactCorrection(entry: Entry) {
    if (entry.sourceClaimId && entry.sourceId) {
      const asset = resolveTimelineSourceAsset(entry.sourceId, assets);
      const purpose = asset?.purpose ?? (/insurance/i.test(entry.category) ? 'insurance' : 'medical');
      router.push({ pathname: '/review', params: { purpose, ...(asset ? { assetId: asset.id } : {}), sourceId: asset?.serverSourceId ?? entry.sourceId, claimId: entry.sourceClaimId } });
      return;
    }
  ...
  async function saveFactCorrection() {
    ...
    try {
      ...
      setEditingFactId(null); setPendingFactFocusId(nextId);
    } catch {
  ...
  function renderGroupedFact(member: Entry, index: number) {
    if (member.kind !== 'fact') return null;
    return <View key={member.id} style={s.groupFact}>
      <View style={s.groupFactHeading}><Text style={s.groupFactIndex}>{String(index + 1).padStart(2, '0')}</Text><View style={{ flex: 1 }}><Text style={s.groupFactCategory}>{member.category.toUpperCase()}</Text><Text style={s.groupFactTitle}>{member.title}</Text><Text style={s.groupFactValue}>{member.detail}</Text></View></View>
      <Text style={s.groupFactSource}>{member.source} · {member.date}</Text>
      {member.note && <Text style={s.noteText}>{member.note}</Text>}
      {member.validUntil && <Text style={s.versionNote}>This earlier value was kept when you corrected the record.</Text>}
      {member.supersedesId && <Text style={s.versionNote}>This is your current correction. The earlier value and its source remain in history.</Text>}
      {editingFactId === member.nodeId.slice('fact:'.length) && !member.sourceClaimId && <View style={s.correctionBox}><Text style={s.sourceLabel}>SAVE A CORRECTED VERSION</Text><Text style={s.correctionHelp}>The current value will remain as an earlier version in your timeline.</Text><TextInput value={editLabel} onChangeText={setEditLabel} placeholder="Detail name" placeholderTextColor={C.faint} style={s.correctionInput} maxLength={120} /><TextInput value={editValue} onChangeText={setEditValue} placeholder="Corrected value" placeholderTextColor={C.faint} style={s.correctionInput} maxLength={500} multiline /><View style={s.correctionActions}><Pressable accessibilityRole="button" disabled={savingFactCorrection} onPress={() => void saveFactCorrection()} style={[s.correctionSave, savingFactCorrection && { opacity: .55 }]}><Text style={s.correctionSaveText}>{savingFactCorrection ? 'SAVING VERSION…' : 'SAVE NEW VERSION'}</Text></Pressable><Pressable accessibilityRole="button" disabled={savingFactCorrection} onPress={() => setEditingFactId(null)} style={s.correctionCancel}><Text style={s.correctionCancelText}>Cancel</Text></Pressable></View></View>}
      <View style={s.groupFactActions}>
        {!member.validUntil && <Pressable accessibilityRole="button" style={s.actionButton} onPress={() => reviewFactCorrection(member)}><Text style={s.actionText}>CORRECT VALUE</Text></Pressable>}
        <Pressable accessibilityRole="button" style={s.actionButton} onPress={() => void openSource(member)}><Text style={s.actionText}>SOURCE + EVIDENCE</Text></Pressable>
        {member.supersedesId && <Pressable accessibilityRole="button" style={s.actionButton} onPress={() => focusLinkedItem(`fact:${member.supersedesId}`)}><Text style={s.actionText}>PREVIOUS VALUE</Text></Pressable>}
        {!member.validUntil && <Pressable accessibilityRole="button" style={s.actionButton} onPress={() => askAbout(member)}><Text style={s.actionText}>ASK NURA</Text></Pressable>}
        <Pressable accessibilityRole="button" style={s.actionButton} onPress={openLinkFlow}><Text style={s.actionText}>＋ CONNECT</Text></Pressable>
      </View>
    </View>;
  }
  ...
  return <View style={s.page}><ScrollView ref={scrollViewRef} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
    ...
    {view === 'timeline' && <>
      ...
      {ready && listedTotal === 0 && <View style={s.emptyCard}><View style={s.emptyIcon}><Text style={s.emptyIconText}>＋</Text></View><Text style={s.emptyTitle}>{filter === 'Documents' ? 'No documents saved yet' : 'Start with one piece of your story'}</Text><Text style={s.emptyBody}>{filter === 'Documents' ? 'Add a report or image when you are ready. It will stay connected to the information you approve.' : 'Add a health detail or a record. Nura will show what was saved and where it came from.'}</Text><Pressable style={s.primaryButton} onPress={() => router.push('/intake')}><Text style={s.primaryButtonText}>ADD A HEALTH RECORD</Text></Pressable></View>}
      {ready && listedTotal > 0 && visibleEntries.length === 0 && <View style={s.emptyCard}><Text style={s.emptyTitle}>No {filter.toLowerCase()} in this history yet</Text><Text style={s.emptyBody}>Try another filter, or add a record when you are ready.</Text><Pressable style={s.primaryButton} onPress={() => { setFilter('Everything'); router.push('/intake'); }}><Text style={s.primaryButtonText}>ADD A HEALTH RECORD</Text></Pressable></View>}
      {timelineSections.map((section) => <React.Fragment key={section.key}>
        <View style={s.yearHeader} accessibilityLabel={`${section.year ?? 'Undated'} · ${section.entries.length} ${filter === 'Documents' ? 'saved documents' : 'captured events'}`}>
          <Text style={s.yearHeaderText}>{section.year ?? 'DATE NOT RECORDED'} <Text style={s.yearHeaderCount}>· {section.entries.length} {filter === 'Documents' ? 'SAVED DOCUMENTS' : 'CAPTURED EVENTS'}</Text></Text>
          <View style={s.yearHeaderRule} />
        </View>
        ...
        return <View key={entry.id} ref={(node) => { if (node) timelineEntryRefs.current.set(entry.id, node); else timelineEntryRefs.current.delete(entry.id); }} style={s.timelineRow}>
          ...
          <View style={s.eventWrap}><View style={[s.eventCard, { backgroundColor: C.white, borderColor: tint.line, borderLeftColor: tint.node, borderLeftWidth: 3 }, selected && { borderColor: tint.node, backgroundColor: '#FBF6FA' }]}>
            ...
            {expanded === entry.id && <View style={s.expanded}><View style={s.divider} />{entry.members ? <><Text style={s.groupSectionLabel}>VALUES IN THIS REPORT · {entry.members.length}</Text>{entry.members.map((member, memberIndex) => renderGroupedFact(member, memberIndex))}</> : <><Text style={s.sourceLabel}>{entry.kind === 'visit' ? 'VISIT SOURCE' : 'SOURCE'}</Text><Text style={s.sourceText}>{entry.source} · {entry.date}</Text>{entry.note && <Text style={s.noteText}>{entry.note}</Text>}
          ...
          {index < section.entries.length - 1 && <View style={s.rowSpacer} />}
        </View>;
      })}
      </React.Fragment>)}
...
const s = StyleSheet.create({
  ...
  groupSectionLabel: { color: C.lilacInk, fontSize: 8, fontWeight: '700', letterSpacing: 1, marginBottom: 7 }, groupFactCategory: { color: C.lilacInk, fontSize: 7, fontWeight: '700', letterSpacing: .7, marginBottom: 2 }, groupFact: { backgroundColor: '#F8F5FA', borderWidth: 1, borderColor: '#E9E1EC', borderRadius: 12, padding: 10, marginTop: 7 }, groupFactHeading: { flexDirection: 'row', alignItems: 'flex-start', gap: 8 }, groupFactIndex: { color: C.lilacInk, fontSize: 8, fontWeight: '700', letterSpacing: .6, paddingTop: 2 }, groupFactTitle: { color: C.ink, fontSize: 11, lineHeight: 15, fontWeight: '600' }, groupFactValue: { color: C.muted, fontSize: 10, lineHeight: 15, marginTop: 2 }, groupFactSource: { color: C.faint, fontSize: 8, lineHeight: 12, marginTop: 6 }, groupFactActions: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 8 },
});
