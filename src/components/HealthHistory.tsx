...
const HISTORY_NODE: NodePalette = { node: '#1767E9', pale: '#EAF1FF', line: '#A9C7FF', accent: '#185BC8' };
    ...
    {view === 'timeline' && <>
      ...
      {visibleEntries.map((entry, index) => {
        ...
        return <View key={entry.id} style={s.timelineRow}>
          ...
          <View style={s.axis}><View style={[s.axisLine, { backgroundColor: HISTORY_NODE.line }]} /><Pressable accessibilityRole="button" accessibilityState={{ selected }} accessibilityLabel={`${selected ? 'Selected' : 'Select'} ${entry.title}`} onPress={() => toggleDetails(entry.id)} style={({ pressed }) => [s.nodeHalo, { backgroundColor: selected ? HISTORY_NODE.pale : '#FFFFFF', borderColor: selected ? HISTORY_NODE.accent : 'transparent', borderWidth: selected ? 1.5 : 0 }, pressed && s.nodePressed]}><Animated.View style={[s.node, { backgroundColor: HISTORY_NODE.node, borderColor: HISTORY_NODE.line, shadowColor: HISTORY_NODE.accent, transform: [{ scale: nodeScaleFor(entry.nodeId) }] }]}><Text style={s.nodeGlyph}>{glyph}</Text></Animated.View></Pressable></View>
          <View style={s.eventWrap}><View style={[s.eventCard, selected && { borderColor: HISTORY_NODE.accent, backgroundColor: '#F4F8FF' }]}>
          ...
          <View style={s.linkDiagram}>
            <View style={s.linkRail}>
              <Pressable accessibilityRole="button" accessibilityLabel={`Open ${nodeTitle(link.from, entries, topics)} in history`} onPress={() => focusLinkedItem(link.from)} style={s.connectionNodeTouch}>
                <View style={[s.connectionNodeHalo, { backgroundColor: HISTORY_NODE.pale, borderColor: HISTORY_NODE.line }]}><View style={[s.connectionNodeCore, { backgroundColor: HISTORY_NODE.node, borderColor: HISTORY_NODE.line }]}><Text style={s.connectionNodeGlyph}>{connectionGlyph(link.from, entries, topics)}</Text></View></View>
              </Pressable>
              ...
              <Pressable accessibilityRole="button" accessibilityLabel={`Open ${nodeTitle(link.to, entries, topics)} in history`} onPress={() => focusLinkedItem(link.to)} style={s.connectionNodeTouch}>
                <View style={[s.connectionNodeHalo, { backgroundColor: HISTORY_NODE.pale, borderColor: HISTORY_NODE.line }]}><View style={[s.connectionNodeCore, { backgroundColor: HISTORY_NODE.node, borderColor: HISTORY_NODE.line }]}><Text style={s.connectionNodeGlyph}>{connectionGlyph(link.to, entries, topics)}</Text></View></View>
              </Pressable>
            </View>
      ...
      <View style={s.domainGraph}>
        ...
        {profileDomains.map((domain) => {
          ...
          return <View key={domain.id} style={s.domainBlock}>
            <Pressable accessibilityRole="button" accessibilityState={{ selected, expanded: selected }} accessibilityLabel={domain.label + ', ' + (count ? detail : 'no saved information yet')} onPress={() => { if (!reducedMotion) LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut); setSelectedNode(selected ? null : domainNodeId); setExpanded(null); }} style={({ pressed }) => [s.domainButton, selected && s.domainButtonSelected, pressed && s.choicePressed]}>
              <View style={s.domainNodeHalo}><Animated.View style={[s.domainNodeCore, { transform: [{ scale: nodeScaleFor(domainNodeId) }] }]}><Text style={s.domainGlyph}>{domain.glyph}</Text></Animated.View></View>
