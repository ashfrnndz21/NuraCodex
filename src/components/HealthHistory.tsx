...
import { getSourceClaims, type CandidateClaim, type LocalSource } from '../services/intakeClient';
...
type Props = { name: string; ready: boolean; storageError: string | null; facts: HealthFact[]; assets: IntakeAsset[]; treatments: TreatmentRecord[]; visits: HealthVisit[]; topics: HealthTopic[]; links: HealthLink[]; addLink: (from: string, to: string, label: string, relationType?: HealthLinkRelation) => HealthLink | null; removeLink: (id: string) => void; correctFact: (id: string, label: string, value: string) => Promise<HealthFact | null>; initialFocusId?: string };
const C = { canvas: '#F6F4F7', white: '#FFFFFF', ink: '#24232B', muted: '#777480', faint: '#A29DA9', border: '#E5E1E9', cobalt: '#1264F5', cobaltDeep: '#0D54D5', blueLine: '#B8CFFF', bluePale: '#EAF1FF', plum: '#483250', lilac: '#E9DDF0', lilacInk: '#735A83', mint: '#DFF2EA', peach: '#F2DDD6', track: '#E8E6EC' };
const domains = ['Biometrics', 'Medical history', 'Records', 'Treatment', 'Lifestyle', 'Care + insurance'];
const timelineFilters = ['Everything', 'Documents', 'Care', 'Treatment', 'Vitals'] as const;
...
const NODE_COLORS: Record<'record' | 'care' | 'treatment' | 'vitals' | 'life' | 'topic', NodePalette> = {
  record: { node: '#1767E9', pale: '#EAF1FF', line: '#A9C7FF', accent: '#185BC8' },
  care: { node: '#8562A8', pale: '#F1EAF6', line: '#C8B4D9', accent: '#72508E' },
  treatment: { node: '#C8863D', pale: '#FBF0DE', line: '#E7C47E', accent: '#94611F' },
  vitals: { node: '#278B95', pale: '#E7F4F3', line: '#9BCFCC', accent: '#26767C' },
  life: { node: '#4B9474', pale: '#E8F3EC', line: '#ABD3BB', accent: '#367855' },
  topic: { node: '#8B6AA6', pale: '#F0EAF4', line: '#CBB8D7', accent: '#75548F' },
};
function entryPalette(entry: Entry): NodePalette {
  ...
  if (/vital|biometric|blood pressure|blood sugar|cholesterol|weight|height|lab result/.test(category)) return NODE_COLORS.vitals;
...
export function HealthHistory({ name, ready, storageError, facts, assets, treatments, visits, topics, links, addLink, removeLink, correctFact, initialFocusId }: Props) {
  ...
  async function openSource(entry: Entry) {
    ...
    const requestId = sourceId ?? asset?.id ?? entry.id;
  ...
  return <View style={s.page}><ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
    ...
    {view === 'timeline' && <>
      <View style={s.profileCard}><View style={s.profileCopy}><Text style={s.profileOverline}>YOUR HEALTH HISTORY</Text><Text style={s.profileTitle}>{captured ? `${captured} captured ${captured === 1 ? 'item' : 'items'}` : 'Your story starts here'}</Text><Text style={s.profileDetail}>{name.trim() ? `A growing record for ${name.trim()}.` : 'Records and details you add will appear here, with their source.'}</Text></View><Pressable style={s.profileOrb} onPress={() => changeView('map')}><View style={s.orbRing}><View style={s.orbCore} /></View></Pressable></View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterChips}>{timelineFilters.map((item) => <Pressable key={item} accessibilityRole="button" accessibilityState={{ selected: filter === item }} onPress={() => { setFilter(item); setExpanded(null); }} style={[s.filterChip, filter === item && s.filterChipSelected]}><Text style={[s.filterChipText, filter === item && s.filterChipTextSelected]}>{item}</Text></Pressable>)}</ScrollView>
      ...
      {visibleEntries.map((entry, index) => {
        ...
        return <View key={entry.id} style={s.timelineRow}>
          ...
          <View style={s.axis}><View style={[s.axisLine, { backgroundColor: tint.line }]} /><Pressable accessibilityRole="button" accessibilityState={{ selected }} accessibilityLabel={`${selected ? 'Selected' : 'Select'} ${entry.title}`} onPress={() => toggleDetails(entry.id)} style={({ pressed }) => [s.nodeHalo, { backgroundColor: selected ? tint.pale : '#FFFFFF', borderColor: selected ? tint.accent : 'transparent', borderWidth: selected ? 1.5 : 0 }, pressed && s.nodePressed]}><Animated.View style={[s.node, { backgroundColor: tint.node, borderColor: tint.line, shadowColor: tint.accent, transform: [{ scale: nodeScaleFor(entry.nodeId) }] }]}><Text style={s.nodeGlyph}>{glyph}</Text></Animated.View></Pressable></View>
          <View style={s.eventWrap}><View style={[s.eventCard, selected && { borderColor: tint.accent, backgroundColor: tint.pale }]}>
            <Pressable accessibilityRole="button" accessibilityState={{ expanded: expanded === entry.id, selected }} onPress={() => toggleDetails(entry.id)} style={({ pressed }) => [s.eventSummary, pressed && { opacity: .92 }]}>
              <View style={s.eventMeta}><View style={[s.kindDot, { backgroundColor: tint.accent }]} /><Text style={[s.eventKind, { color: tint.accent }]}>{entry.kind === 'fact' ? entry.category.toUpperCase() : `${entry.category} · SAVED`}</Text>{entry.kind === 'fact' && entry.validUntil && <Text style={s.versionBadge}>PREVIOUS VERSION</Text>}{entry.kind === 'fact' && entry.supersedesId && <Text style={s.versionBadge}>CORRECTED BY YOU</Text>}<Text style={s.eventDate}>{entry.date}</Text></View>
        ...
        return <View key={link.id} style={s.linkCard}>
          <View style={s.linkDiagram}>
            <View style={s.linkRail}>
              ...
              <View style={s.linkRailGap}><Animated.View style={[s.linkRailLine, { transform: [{ scaleY: routeMotion }] }]} /><View style={s.linkRailDot} /></View>
    ...
    {view === 'map' && <View style={s.mapPanel}><View style={s.mapHero}><View style={s.mapOrb}><View style={s.orbCore} /></View><Text style={s.mapName}>{name.trim() || 'You'}</Text><Text style={s.mapCaption}>YOUR PROFILE · BUILT FROM YOUR INPUT</Text></View>{domains.map((domain, index) => { const count = [facts.length ? 1 : 0, topics.length ? topics.length : 0, assets.length, facts.length ? facts.length : 0, 0, 0][index]; return <View key={domain} style={s.domainRow}><View style={[s.domainNode, count > 0 && s.domainNodeActive]}><Text style={[s.domainNumber, count > 0 && s.domainNumberActive]}>{String(index + 1).padStart(2, '0')}</Text></View><View style={{ flex: 1 }}><Text style={s.domainTitle}>{domain}</Text><Text style={s.domainSub}>{count ? `${count} saved item${count === 1 ? '' : 's'} to explore` : 'Add when you are ready'}</Text></View><View style={[s.domainTrack, count > 0 && s.domainTrackActive]} /></View>})}<Text style={s.mapFootnote}>This profile is an organizing view. It does not infer a diagnosis or relationship between records.</Text></View>}
    </Animated.View>
        ...
        {sourceDetail.sourceId && !sourceDetail.loading && !sourceDetail.error && <>
          ...
          {sourceDetail.claims.map((claim) => {
            ...
            return <View key={claim.id} style={[s.sourceClaim, sourceDetail.claimId === claim.id && s.sourceClaimSelected]}>
              ...
              <View style={s.sourceClaimTop}><View style={{ flex: 1 }}><Text style={s.choiceTitle}>{claim.label}</Text><Text style={s.sourceClaimValue}>{claim.value}{claim.unit ? ` ${claim.unit}` : ''}{claim.effectiveAt ? ` · ${readableDate(claim.effectiveAt)}` : ''}</Text></View><Text style={[s.claimState, stateStyle]}>{state}</Text></View>
...
const s = StyleSheet.create({
  ...
  segment: { flexDirection: 'row', borderRadius: 17, padding: 4, backgroundColor: '#EDEAF0', marginTop: 20, borderWidth: 1, borderColor: C.border }, segmentButton: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 11, borderRadius: 13 }, segmentSelected: { backgroundColor: C.cobalt, shadowColor: C.cobalt, shadowOpacity: 0.14, shadowRadius: 7, shadowOffset: { width: 0, height: 3 } }, segmentText: { color: C.muted, fontSize: 13, fontWeight: '500' }, segmentTextSelected: { color: C.white }, warning: { backgroundColor: '#FFF4E5', borderRadius: 12, padding: 12, marginTop: 14 }, warningText: { color: '#80551D', fontSize: 11, lineHeight: 16 },
  profileCard: { backgroundColor: C.white, borderRadius: 21, padding: 16, marginTop: 18, borderWidth: 1, borderColor: C.border, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', overflow: 'hidden' }, profileCopy: { flex: 1, paddingRight: 8 }, profileOverline: { color: C.lilacInk, fontSize: 8, fontWeight: '700', letterSpacing: 1.6 }, profileTitle: { color: C.ink, fontSize: 18, fontWeight: '600', marginTop: 7 }, profileDetail: { color: C.muted, fontSize: 11, lineHeight: 16, marginTop: 4, maxWidth: 240 }, profileOrb: { width: 78, height: 78, alignItems: 'center', justifyContent: 'center' }, orbRing: { width: 67, height: 67, borderRadius: 36, borderWidth: 1, borderColor: '#DCE5FA', backgroundColor: '#F7F9FF', alignItems: 'center', justifyContent: 'center' }, orbCore: { width: 34, height: 34, borderRadius: 18, backgroundColor: C.cobalt, borderWidth: 2, borderColor: '#AFCBFF', shadowColor: C.cobalt, shadowOpacity: .25, shadowRadius: 10, shadowOffset: { width: 0, height: 2 } }, filterRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 25, marginBottom: 8 }, filterTitle: { color: C.ink, fontSize: 10, fontWeight: '700', letterSpacing: 1.2 }, filterSub: { color: C.faint, fontSize: 10, marginTop: 4 }, filterButton: { backgroundColor: C.white, borderRadius: 18, borderWidth: 1, borderColor: C.border, paddingHorizontal: 12, paddingVertical: 8 }, filterButtonText: { color: C.lilacInk, fontSize: 10, fontWeight: '600' }, filterChips: { flexDirection: 'row', gap: 7, paddingBottom: 13 }, filterChip: { paddingHorizontal: 13, paddingVertical: 8, borderRadius: 18, borderWidth: 1, borderColor: C.border, backgroundColor: C.white }, filterChipSelected: { backgroundColor: C.bluePale, borderColor: '#AFCBFF' }, filterChipText: { color: C.muted, fontSize: 10, fontWeight: '500' }, filterChipTextSelected: { color: C.cobaltDeep, fontWeight: '700' },
  timelineRow: { flexDirection: 'row', alignItems: 'stretch', minHeight: 116 }, dateCol: { width: 48, paddingTop: 10, alignItems: 'flex-start' }, dateDay: { color: C.ink, fontSize: 16, fontWeight: '600', letterSpacing: -.4 }, dateMonth: { color: C.muted, fontSize: 9, fontWeight: '600', letterSpacing: 1, marginTop: 1 }, dateYear: { color: C.faint, fontSize: 9, marginTop: 5 }, axis: { width: 48, alignItems: 'center', position: 'relative' }, axisLine: { position: 'absolute', top: 0, bottom: 0, width: 2, backgroundColor: C.blueLine }, nodeHalo: { marginTop: 4, width: 48, height: 48, borderRadius: 25, backgroundColor: '#EAF1FF', alignItems: 'center', justifyContent: 'center', zIndex: 1 }, nodePressed: { transform: [{ scale: motion.pressScale }] }, node: { width: 37, height: 37, borderRadius: 20, alignItems: 'center', justifyContent: 'center', backgroundColor: C.cobalt, borderWidth: 1.5, borderColor: '#8DB4FF', shadowColor: C.cobalt, shadowOpacity: .23, shadowRadius: 6, shadowOffset: { width: 0, height: 2 } }, nodeGlyph: { color: C.white, fontSize: 17, fontWeight: '500' }, eventWrap: { flex: 1, paddingBottom: 13 }, eventCard: { backgroundColor: C.white, borderColor: C.border, borderWidth: 1, borderRadius: 17, paddingHorizontal: 13, paddingVertical: 12, shadowColor: '#30263D', shadowOpacity: .035, shadowRadius: 8, shadowOffset: { width: 0, height: 3 } }, eventCardOpen: { borderColor: '#AFCBFF', shadowOpacity: .08 }, eventSummary: { borderRadius: 12 }, cardPressed: { opacity: .96, transform: [{ scale: motion.pressScale }] }, eventMeta: { flexDirection: 'row', alignItems: 'center', gap: 6 }, kindDot: { width: 7, height: 7, borderRadius: 4 }, kindFact: { backgroundColor: C.cobalt }, kindFile: { backgroundColor: '#8B6CAB' }, eventKind: { color: C.lilacInk, fontSize: 8, fontWeight: '700', letterSpacing: .75, flex: 1 }, eventDate: { color: C.faint, fontSize: 8 }, eventTitle: { color: C.ink, fontSize: 15, fontWeight: '600', lineHeight: 20, marginTop: 7 }, eventDetail: { color: C.muted, fontSize: 11, lineHeight: 16, marginTop: 3 }, versionBadge: { color: '#7A5C84', fontSize: 6, fontWeight: '800', letterSpacing: .5, backgroundColor: '#F0E8F4', overflow: 'hidden', borderRadius: 7, paddingHorizontal: 5, paddingVertical: 4 }, versionNote: { color: '#705583', fontSize: 9, lineHeight: 14, marginTop: 6, backgroundColor: '#F5EFF8', padding: 8, borderRadius: 10 }, correctionBox: { backgroundColor: '#F7F5FA', borderColor: '#E4DCE9', borderWidth: 1, borderRadius: 13, padding: 10, marginTop: 10 }, correctionHelp: { color: C.muted, fontSize: 9, lineHeight: 13, marginTop: 4 }, correctionInput: { minHeight: 38, borderRadius: 10, borderWidth: 1, borderColor: C.border, backgroundColor: C.white, paddingHorizontal: 10, paddingVertical: 8, color: C.ink, fontSize: 10, marginTop: 7 }, correctionActions: { gap: 7, marginTop: 8 }, correctionSave: { minHeight: 39, borderRadius: 11, backgroundColor: C.cobalt, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 10 }, correctionSaveText: { color: C.white, fontSize: 8, fontWeight: '800', letterSpacing: .6 }, correctionCancel: { minHeight: 33, alignItems: 'center', justifyContent: 'center' }, correctionCancelText: { color: C.muted, fontSize: 9 }, notAnalyzed: { color: '#8A7298', fontSize: 9, marginTop: 6 }, repeatText: { color: '#9B651D', fontSize: 9, lineHeight: 14, marginTop: 5 }, viewDetails: { color: C.cobaltDeep, fontSize: 8, letterSpacing: .9, fontWeight: '700', marginTop: 11 }, expanded: { marginTop: 10 }, divider: { height: 1, backgroundColor: C.border, marginBottom: 9 }, sourceLabel: { color: C.faint, fontSize: 8, fontWeight: '700', letterSpacing: 1.2 }, sourceText: { color: C.muted, fontSize: 10, marginTop: 4 }, noteText: { color: C.muted, fontSize: 10, lineHeight: 15, marginTop: 6 }, cardActions: { gap: 8, marginTop: 12 }, sourceDetailAction: { alignItems: 'center' }, secondaryCardActions: { flexDirection: 'row', gap: 8 }, actionButton: { backgroundColor: C.bluePale, paddingHorizontal: 11, paddingVertical: 8, borderRadius: 14 }, actionText: { color: C.cobaltDeep, fontSize: 10, fontWeight: '600' }, rowSpacer: { height: 1 },
