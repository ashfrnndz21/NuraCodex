...
| **P3 · Persistent storage and sync** | **Partial:** native local SQLite/SQLCipher state exists; browser preview state is synthetic/in-memory; cloud identity, sync and recovery do not exist. | Encrypted production database/object storage, key management, backups/recovery, multi-device sync, conflict/version handling, retention and account closure. | Restore, conflict, retention and deletion propagation drills pass across primary data, derived indexes, caches, objects and backups according to policy. |
