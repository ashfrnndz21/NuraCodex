...
| **P4 · Multimodal record pipeline** | **Partial:** single-file PDF/image extraction and a consent-gated local-demo video path connect to candidate review; batch intake, one-pass review of multiple files plus self-report, perceptual duplicate suggestions, audio transcription/provider analysis and production object processing are absent. | Multi-file health intake with self-reported notes, safe PDF/image/video/audio processing, provenance to page/region/timecode, exact and suggested duplicate handling, active-person confirmation, evidence review, retry/cancel and retention controls. | Synthetic fixtures cover batch and each supported format plus failure/duplicate/conflict paths; provider transfer follows explicit consent; activity, citations and timecodes are accurate; no unreviewed candidate enters confirmed memory. |
...
|---|---|---|---|
| 1 | In progress | Dynamic tracking topics/map and first cited synthesis; profile persistence loader now has SQLite close/reopen coverage | Email/mobile login and recovery are absent; complete auth-first storyboard and implementation; batch multi-source intake with self-report; evidence-level review/save; timeline and re-entry; native/device motion acceptance |
