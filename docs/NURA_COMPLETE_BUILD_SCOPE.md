...
| **P4 · Multimodal record pipeline** | **Partial:** PDF/image extraction and a consent-gated video path are connected in the local demo. Video selection is available from file/photo pickers; after per-file confirmation, the local service samples up to six stills from clips up to three minutes, sends only those stills to the configured model, and returns timestamped candidate claims to the existing review screen. Local audio-format/duration preflight and timestamped quote mapping exist as unconnected helpers; audio intake, transcription, and provider analysis are absent. Batch intake, perceptual duplicate suggestions and production object processing are absent. | Safe PDF/image/video/audio processing, provenance to page/region/timecode, exact and suggested duplicate handling, owner confirmation, review queue, retry/cancel and retention controls. | Synthetic fixtures cover each supported format and failure/duplicate/conflict paths; the video journey must verify sampled-frame provider transfer only after consent, visible activity/timecodes, cancellation, exact duplicate handling and candidate-only review. No unreviewed candidate enters confirmed memory. |
