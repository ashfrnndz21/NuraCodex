...
- Native profile, focus areas, self-reported facts, selected files, links and Ask conversation persist in local SQLite. Browser demo starts with synthetic examples and saves demo state in localStorage plus selected file bytes in IndexedDB so journey tests survive refresh. This browser storage is not an encrypted account vault; the app warns users to use fictional data only and exposes clear-data controls.
