...
- Current application remains behind this target: it opens at profile setup without login/TAC; health upload is still separate from the topic chooser; no unified multi-file-plus-note onboarding batch exists yet. This storyboard change does not count as implemented product capability.
...
- **Next implementation slice:** establish real/demo-explicit authentication and the active-person boundary, then connect one batch intake through event-driven extraction, evidence review, approved persistence and the health timeline. Insurance comparison with health context remains a later, separately consented stage.
