...
| Intake | PDF/image transfer and review are connected. Video is also connected in code: the Medical Registry file/photo pickers accept clips; Review explains the per-file provider transfer and requires the person to continue; the local service samples up to six stills from clips up to 3 minutes, sends sampled stills to the configured vision model, and returns timestamped claims to the existing accept/edit/reject screen. The medical intake now accepts an optional topic-tagged, user-written description alongside the selected files; it persists as a draft, stays labeled as the user's words, and can be edited, added to the profile, or removed in Review. Local audio-format/duration checks and timestamped quote-to-segment mapping have unit coverage, but are not connected to intake or transcription. Synthetic PDF extraction, review/acceptance, exact-duplicate detection and later Ask retrieval passed live checks. | Real email/mobile verification and owner isolation remain open; AI still processes files one at a time with per-file consent, and does not interpret the self-report. Add one consented batch run with event-driven per-file extraction, duplicate/date/conflict checks and a single approved-memory review/save; then verify retry/cancel/restart and mobile/reduced-motion behavior. |
