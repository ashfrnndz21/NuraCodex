...
## Executive progress and delivery plan — 27 September 2026
...
|---|---|---|
| **M1 · Profile and first history** | In progress; an isolated synthetic run staged two PDFs and one note, used the local-only note organizer with consent, accepted one dated blood-pressure suggestion, dismissed one undated dizziness suggestion, saved once, and returned to a three-event timeline that survived browser refresh. Both PDFs correctly remained saved and unanalyzed. | Run consented, event-backed extraction of both synthetic PDFs; review their source-linked claims together with the note; prove cross-source duplicate/conflict behavior, edit/reject/unclear/date decisions, one save, source navigation and re-entry. Then native, reduced-motion and identity-isolation checks. |
...
The implementation keeps the Blueprint v2 plum-to-rose scene and replaces the orbit/rays with a warm-paper overview. It separates topic preferences from records, groups saved details beneath source records, uses real dates/source state only, and keeps topic detail editing distinct from add/remove selection. A 390 pt browser check passed; an isolated preview also observed six selected areas and one nested detail. At 360 pt, the overview fits without document-level horizontal overflow. At a 430 pt browser viewport the preview's phone frame stays capped at 390 pt, so that check does not establish a true 430 pt device layout. Native 430 pt, reduced-motion, tap-target and accessibility checks remain open; D0 is still in progress.

**Current build target:** finish M1's file-backed review journey and source-linked history/re-entry, while closing D0's true 430 pt device, reduced-motion, keyboard/tap-target and accessibility checks. The event-date branch still shows an explicitly blank event date as “Date not stated” without substituting a report date. The onboarding implementation uses the evidence-first overview; no pending-review queue or timestamp is shown unless backed by persisted data.
...
|---|---|---|---|
| **D0 · Health-profile overview** | Replace the onboarding orbit with a source-first profile overview, while preserving the topic selector and topic-detail editor. | **Approved; implementation and 360 pt browser fit are observed.** `app/index.tsx` has distinct focus-area and grouped-record sections; source-backed helper and regression tests cover source status, type labels and grouping. The web phone shell caps at 390 pt, so true 430 pt device fit is not evidenced. | True 430 pt device width, reduced-motion, all-nine-area, keyboard/tap-target and accessibility checks; then integrate into M1. |
| **M1 · First profile and history** | Login rehearsal → required profile → one batch of health files/notes → real processing events → item review → one save → dated/source-linked history → leave and re-enter. | **In progress.** Isolated synthetic run: staged two PDFs plus a note, consented to local-only note organization, accepted a dated blood-pressure suggestion, dismissed an undated dizziness suggestion, saved once, and saw one measurement plus two unprocessed PDF sources in a 3-event history after refresh. PDFs were not sent for analysis. Cross-source dedupe is not yet proven. | Consent-backed event extraction from both synthetic files; source-matched review of all items; clear/edit/reject/unclear/date paths; cross-source duplicate/conflict behavior; one save; source navigation and re-entry. Then native/reduced-motion and identity/isolation remain for full M1 acceptance. |
...
### Reference review and M1 browser journey — 27 September 2026, 00:26 +08

- **Design direction from the supplied examples:** keep the warm paper/plum visual language, use restrained translucent cards to group topics and evidence, and show a short plain-language answer before expandable details. Keep “what the record says,” “what it can mean,” “what is unclear” and cited sources visibly distinct. Analysis motion should reflect real service events only; no fake thinking traces, unsupported counts, hidden reasoning, prognostic charts, health scores or direct treatment advice copied from mockups. Added these as guardrails in `docs/EXPERIENCE_DESIGN_SPEC.md` and the complete-build contract.
- **M1 synthetic browser evidence:** on isolated `127.0.0.1:8097`, staged two synthetic PDFs plus one fictional note. The two PDFs remained saved and unanalyzed; no provider received them. After per-note local consent, the deterministic local organizer showed a dated blood-pressure suggestion and an undated dizziness suggestion. The rehearsal accepted the former, dismissed the latter, and saved once. `/health` then showed 3 events: one dated measurement and the two distinct, unprocessed PDF sources. Refresh preserved those 3 entries. This does not test duplicate detection between independently extracted files and a note because neither PDF was processed.
- **Overview size check:** at 360 pt, the profile-summary page had no document-level horizontal overflow. At 430 pt the web preview retained its 390 pt phone frame, so a true 430 pt device-width check remains open. Native rendering, reduced motion, tap targets and assistive-technology checks are also open. Expo logged existing React Native Web deprecation warnings for shadow style props and `pointerEvents`; they did not block the preview.
- **Checks on current source:** typecheck passed; lint passed with exit 0; latest slices **77/77**; review accessibility **2/2**; repository/integration suite **254/254** after permitting its loopback-only synthetic servers; `git diff --check` passed. The initial sandboxed repository-suite attempt failed only the 3 loopback tests (251 passed); the permitted rerun passed all 254. No external provider was called.
- **Progress:** **0/11 accepted stories, 0/8 passed production gates, 0/19 accepted packages (0%)**. M1 remains in progress. No whole-user test is ready.
- **Next slice:** process both synthetic PDFs through the explicit-consent, real-event intake path, then review their evidence beside the note and verify cross-source duplicates/conflicts before one save and timeline re-entry. Keep M2/M8 available for independent work after this M1 check.
- **Open scope:** all eleven stories and eight production gates remain short of full acceptance; M10, P7 and P8 are unbuilt. Production identity/isolation, secure sync, durable consent/deletion, hosted operations, security, clinical/legal review and release evidence remain required.
