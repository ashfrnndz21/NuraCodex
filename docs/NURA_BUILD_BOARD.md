...
| Intake | PDF/image transfer and review are connected. Video is also connected in code: the Medical Registry file/photo pickers accept clips; the local service samples up to six stills from clips up to 3 minutes and returns timestamped claims. Medical intake accepts an optional topic-tagged self-report alongside multiple files; it persists separately as a user-authored draft. Review now asks once for consent naming every staged file, processes them sequentially with real per-file activity, continues after an individual failure, saves each file-to-source link before reporting completion, and reopens each linked source without resending its original. One stopped run retains completed files and leaves unstarted files ready. Local audio-format/duration checks and timestamped quote-to-segment mapping have unit coverage, but are not connected to intake or transcription. Synthetic single-file extraction, review/acceptance, exact-duplicate detection and later Ask retrieval passed prior live checks. | Provider-backed email/mobile verification and owner isolation remain open. Batch files still require separate source-by-source claim review: cross-file duplicate/date/topic/conflict analysis, a unified review and one approved-memory save are missing; the self-report is not model-interpreted. Add those controls, then verify retry/cancel/restart and mobile/reduced-motion behavior. |
...
- **Next:** consolidate candidate claims across all sources into one review surface with an explicit approved-memory save; add cross-file duplicate/date/conflict checks. Provider-backed login and owner isolation remain an earlier production prerequisite and are not simulated by this change.
