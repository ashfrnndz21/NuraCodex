...
| Intake | PDF/image transfer and review are connected. Video is also connected in code: the Medical Registry file/photo pickers accept clips; Review explains the per-file provider transfer and requires the person to continue; the local service samples up to six stills from clips up to 3 minutes, sends sampled stills to the configured vision model, and returns timestamped claims to the existing accept/edit/reject screen. Local audio-format/duration checks and timestamped quote-to-segment mapping now have unit coverage, but are not connected to intake or transcription. Synthetic PDF extraction, review/acceptance, exact-duplicate detection and later Ask retrieval passed live checks. | Audio transcription and candidate extraction; a fresh synthetic video end-to-end run through picker → consent → sampling → provider → timestamped review; video duplicate/cancellation/error exercise; batch intake and perceptual duplicate suggestions. |
...
- Added [NURA_HEALTH_HISTORY_DESIGN_OPTIONS.html](NURA_HEALTH_HISTORY_DESIGN_OPTIONS.html), an interactive three-direction design review: a chronological story timeline, a current-picture-plus-history view, and topic-specific time lanes. These are design options awaiting the user's selection, not compiled app screens.
- Product distinction identified for the next design decision: choosing “Joints and movement” or “X-ray or scan” currently saves a tracking/detail topic; it does not select or upload a medical report. The follow-up journey should explicitly separate tracking interests, user-entered/current details, and source-backed historical records. A report is added only from a clear upload action, then extracted as a candidate and confirmed by the user before it is treated as a record.
