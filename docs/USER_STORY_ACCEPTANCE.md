...
|---|---|---|
| 1. Living 720 profile | Partial | The phone-sized onboarding has dynamic focus bubbles, dependent selections, a changing 720 map, profile fields and optional biometrics. A consent-gated first synthesis returns cited selected-profile evidence and unknowns, can recontextualize a user correction, and offers an explicit new-note save. Health intake now supports multiple staged files plus an optional topic-tagged self-report, persisted separately as a draft; review lets the user edit, add it as a user-authored fact, or remove it. The timeline explicitly labels the entry date when no event date was supplied. The note is not AI-interpreted yet. Provider-backed sign-in/recovery, owner isolation, unified consented batch processing, one final review/save point and native/reduced-motion end-to-end checks remain incomplete. |
