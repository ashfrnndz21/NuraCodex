...
| 3. Insurance Registry | Partial | A dedicated registry route accepts policy-purpose uploads, extracts candidate terms with page quotes, gates them through review, and stores accepted terms with source and claim IDs. Ask now returns source-validated typed benefit/limit/exclusion/unclear assessments; a synthetic coverage run completed with policy citations and no unrelated server assertions. Corrected prior terms and user-removed entries are shown under the original source. Users can mark one saved policy as replacing another, compare exact-label accepted terms, and open each compared value's exact saved claim quote. The full two-policy extraction/review/comparison/Ask journey, conflict semantics and selected-health-evidence citation check remain incomplete. |
